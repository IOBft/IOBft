...
namespace ETModel
{
	...
	public sealed class KService : AService
	{
		...
		private static uint IdGenerater = 1000;
        ...
        public override IPEndPoint GetEndPoint(){return (IPEndPoint)this.socket?.LocalEndPoint;}

        public KService(IPEndPoint _ipEndPoint, Action<AChannel> acceptCallback)
		{
			...
			while (socket != null && this.socket.Available > 0)
			{
				...
				switch (flag)
				{
					case KcpProtocalType.SYN:  // accept
						...
						IPEndPoint acceptIpEndPoint = (IPEndPoint)this.tempRemoteEndPoint;
						this.tempRemoteEndPoint = new IPEndPoint(0, 0);
		...
		{
			uint localConn = (uint)RandomHelper.Range(1000, int.MaxValue);
			KChannel oldChannel;
