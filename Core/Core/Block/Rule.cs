...
namespace ETModel
{
    public class Rule : Component
    {
        ...
        protected Dictionary<string, Transfer> transfers = new Dictionary<string, Transfer>();
        ...
        public override void Start()
        {
            ...
            httpRule = Entity.Root.GetComponentInChild<HttpPool>();
        ...
        public Block CreateBlock(Block preblk)
        {
            ...
            blockMgr.DelBlockWithWeight(consensus,preblk.hash, preblk.height);
        ...
        public bool RefTransfer(Block blk)
        {
            try
            {
                lock (transfers)
                {
                    var list = transfers.Values.ToList<Transfer>();
                    list.Sort((Transfer a, Transfer b) => {
                        int rel = a.notice.CompareTo(b.notice);
                        if (rel == 0)
        ...
        public void MinerReward()
        {
            if (httpRule != null)
            {
                Dictionary<string, MinerTask> miners = httpRule.GetMinerReward(out long miningHeight);
                if (miners != null && miningHeight + 3 < height)
                {
                    ...
                    httpRule.DelMiner(miningHeight);
                }
            }
        }
        ...
        public static void P2P_Transfer_Handle(Session session, int opcode, object msg)
        {
            ...
            Transfer transfer = JsonHelper.FromJson<Transfer>(q2p_Transfer.transfer);
